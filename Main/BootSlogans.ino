...
String getRandomBootSlogan()
{
  if (isConfigured(localeSetting))
  {
    ...
    { // check specific locales first because they are preferred (de_CH before de)
      return getRandomElementFromArray(bitcoinSlogans_de_CH, sizeof(bitcoinSlogans_de_CH) / sizeof(bitcoinSlogans_de_CH[0]));
    }
    else if (strncmp(localeSetting, "de", 2) == 0)
    { // matches de_DE and other de_* locales
    ...
    { // matches Spanish (Espanol) and other es_* locales
    ...
    { // matches Dutch (Nederlands) and other nl_* locales
    ...
    { // matches pt* while there's no pt_PT
